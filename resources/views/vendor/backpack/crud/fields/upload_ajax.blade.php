...
@include('crud::fields.inc.wrapper_start')
    ...
    <div class="backstrap-file {{ isset($field['value']) && $field['value']!=null?'d-none':'' }}">
        ...
        <input type="hidden" name="{{ $field['name'] }}"
               value="{{ old(square_brackets_to_dots($field['name'])) ?? $field['value'] ?? $field['default'] ?? '' }}"
        >
        <label class="backstrap-file-label" for="customFile">{{ old(square_brackets_to_dots($field['name'])) ?? $field['value'] ?? $field['default'] ?? '' }}</label>
    </div>
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if (data.success) {
                                ...
                                $('[name="'+fieldName+'"]').val(data.success[0]);
                                inputLabel.html(data.success[0]);
                            } else {
                                alert(data.error);
                            }
                        },

...
<div class="array-container form-group">
    ...
    <table class="table table-sm table-striped m-b-0">
        ...
        <thead>
        <tr>
            @foreach( $field['columns'] as $column => $input )
        ...
        <tr class="array-row clonable" style="display: none;">
            @foreach( $field['columns'] as $column => $input)
                <td>
                    @include('crud::fields.' . $input['type'], ['field' => $input + ['name' => $column,
                    'attributes' => ['data-cell-name' => 'item.' . $column]]])
{{--                    <input class="form-control form-control-sm" type="{{$input['type']}}" data-cell-name="item.{{ $input['label'] }}">--}}
                </td>
            @endforeach
            <td>
